...
export default function AIAssistant() {
  ...
  const [activeComponent, setActiveComponent] = useState('CalcTools');
  const [expandedMenus, setExpandedMenus] = useState({ 'calc': true, 'vlab': false, 'cases': false });

  const toggleMenu = (menu) => setExpandedMenus(prev => ({ ...prev, [menu]: !prev[menu] }));

  useEffect(() => {
    const path = location.pathname;
    if (path.includes('/calc-tools')) setActiveComponent('CalcTools');
    else if (path.includes('/v-lab')) setActiveComponent('VirtualLab');
    else if (path.includes('/cases')) setActiveComponent('ResearchCases');
    else setActiveComponent('CalcTools');
  }, [location.pathname]);

  const [chatInput, setChatInput] = useState('');
  const [chatHistory, setChatHistory] = useState([
    { role: 'ai', text: '您好！我是集成大模型的智能科研导师。我可以帮您阅读最新的 Nature/Science 化学文献，或者辅助构建密度泛函理论(DFT)的输入文件。' }
  ]);
  const chatEndRef = useRef(null);

  const componentNameMap = { 'CalcTools': '化学计算工具', 'VirtualLab': '虚拟实验平台', 'ResearchCases': '科研案例库' };
  ...
  const renderContent = () => {
    switch (activeComponent) {
      case 'CalcTools': return <CalcTools />; case 'VirtualLab': return <VirtualLab />;
      case 'ResearchCases': return <ResearchCases />; default: return <CalcTools />;
    }
  };
      ...
      <div style={styles.workspace}>
        <aside style={styles.sidebar}>
          <div style={styles.menuGroup}>
            <div style={styles.menuHeader} onClick={() => toggleMenu('calc')}><span className="material-symbols-outlined" style={styles.menuIcon}>calculate</span> <span style={styles.menuText}>化学计算工具</span></div>
            {expandedMenus['calc'] && (<div style={styles.subMenuBox}><div style={activeComponent === 'CalcTools' ? styles.subMenuItemActive : styles.subMenuItem} onClick={() => setActiveComponent('CalcTools')}>DFT与分子动力学</div></div>)}
          </div>
          <div style={styles.menuGroup}>
            <div style={styles.menuHeader} onClick={() => toggleMenu('vlab')}><span className="material-symbols-outlined" style={styles.menuIcon}>biotech</span> <span style={styles.menuText}>虚拟实验平台</span></div>
            {expandedMenus['vlab'] && (<div style={styles.subMenuBox}><div style={activeComponent === 'VirtualLab' ? styles.subMenuItemActive : styles.subMenuItem} onClick={() => setActiveComponent('VirtualLab')}>虚拟实验模拟</div></div>)}
          </div>
          <div style={styles.menuGroup}>
            <div style={styles.menuHeader} onClick={() => toggleMenu('cases')}><span className="material-symbols-outlined" style={styles.menuIcon}>article</span> <span style={styles.menuText}>科研案例库</span></div>
            {expandedMenus['cases'] && (<div style={styles.subMenuBox}><div style={activeComponent === 'ResearchCases' ? styles.subMenuItemActive : styles.subMenuItem} onClick={() => setActiveComponent('ResearchCases')}>顶刊文献解析</div></div>)}
          </div>
        </aside>
...
const styles = {
  ...
  subMenuItemActive: { padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', color: '#0284c7', backgroundColor: '#f0f9ff', fontSize: '13px', fontWeight: 'bold' },
