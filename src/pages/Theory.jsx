...
export default function Theory() {
  ...
  const [chatHistory, setChatHistory] = useState([
    { role: 'ai', text: '欢迎来到理论应用舱！这里我们将抽象理论转化为实际生产。关于 MOFs 的光学设计、晶体场理论、或是勒夏特列原理的工业应用，向我开炮吧！' }
  ]);
  const chatEndRef = useRef(null);

  // 映射当前模块名称，给 AI 提供上下文
  const componentNameMap = { 
    'StructureProperty': '组成结构与性能 (构效关系)', 
    'EqShift': '化学平衡移动 (工业条件优化)', 
    'RateControl': '反应速率调控 (催化与动力学)' 
  };
  ...
  const handleChatSubmit = async (e, isButton = false) => {
    ...
    setChatInput('');
    setChatHistory(prev => [...prev, { role: 'user', text: userText }, { role: 'ai', text: '正在深度思考并调取理论模型...' }]);
    ...
    try {
      ...
      setChatHistory(prev => { 
        const newHistory = [...prev]; 
        newHistory[newHistory.length - 1] = { role: 'ai', text: '⚠️ 算力节点连接断开，请重试。' }; 
        return newHistory; 
      });
    }
  };
  ...
  const renderContent = () => {
    switch (activeComponent) {
      case 'StructureProperty': return <StructureProperty />; 
      case 'EqShift': return <EqShift />;
      case 'RateControl': return <RateControl />; 
      default: return <StructureProperty />;
    }
  };
      ...
      <div style={styles.workspace}>
        ...
        <aside style={styles.sidebar}>
          ...
          <div style={styles.menuGroup}>
            <div style={styles.menuHeader} onClick={() => toggleMenu('properties')}>
              <span className="material-symbols-outlined" style={styles.menuIcon}>category</span> 
              <span style={styles.menuText}>组成结构与性能</span>
            </div>
            {expandedMenus['properties'] && (
              <div style={styles.subMenuBox}>
                <div style={activeComponent === 'StructureProperty' ? styles.subMenuItemActive : styles.subMenuItem} onClick={() => setActiveComponent('StructureProperty')}>
                  构效关系推演
                </div>
              </div>
            )}
          ...
          <div style={styles.menuGroup}>
            <div style={styles.menuHeader} onClick={() => toggleMenu('equilibrium')}>
              <span className="material-symbols-outlined" style={styles.menuIcon}>sync_alt</span> 
              <span style={styles.menuText}>化学平衡移动</span>
            </div>
            {expandedMenus['equilibrium'] && (
              <div style={styles.subMenuBox}>
                <div style={activeComponent === 'EqShift' ? styles.subMenuItemActive : styles.subMenuItem} onClick={() => setActiveComponent('EqShift')}>
                  工业条件优化
                </div>
              </div>
            )}
          ...
          <div style={styles.menuGroup}>
            <div style={styles.menuHeader} onClick={() => toggleMenu('rate')}>
              <span className="material-symbols-outlined" style={styles.menuIcon}>tune</span> 
              <span style={styles.menuText}>反应速率调控</span>
            </div>
            {expandedMenus['rate'] && (
              <div style={styles.subMenuBox}>
                <div style={activeComponent === 'RateControl' ? styles.subMenuItemActive : styles.subMenuItem} onClick={() => setActiveComponent('RateControl')}>
                  催化与动力学控制
                </div>
              </div>
            )}
          ...
          <div style={styles.aiBody}>
            {chatHistory.map((msg, index) => (
              <div key={index} style={{ ...styles.aiMessageBase, alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start', backgroundColor: msg.role === 'ai' ? '#f5f3ff' : '#eff6ff', borderColor: msg.role === 'ai' ? '#ddd6fe' : '#bfdbfe' }}>
                <div style={{ fontWeight: 'bold', fontSize: '12px', color: msg.role === 'ai' ? '#7c3aed' : '#2563eb', marginBottom: '4px' }}>
                  {msg.role === 'ai' ? '🤖 AI导师' : '👤 您'}
                </div>
...
const styles = {
  ...
  backBtn: { color: '#ddd6fe', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px', fontWeight: 'bold', padding: '6px 12px', borderRadius: '6px', transition: '0.2s' },
  ...
  menuHeader: { display: 'flex', alignItems: 'center', padding: '10px 12px', borderRadius: '8px', cursor: 'pointer', color: '#475569', transition: '0.2s' },
  ...
  // 激活状态变为高亮紫
  subMenuItemActive: { padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', color: '#7c3aed', backgroundColor: '#f5f3ff', fontSize: '13px', fontWeight: 'bold', transition: '0.2s' },
